...
const language = ({ message, flags, client }) => {
    ...
    if (botRole) {
        if (!userRoles.some(role => role.comparePositionTo(botRole) > 0)) {
            main.sendMessage({
                message, text: `${message.author.username} does not have a role high enough to change language`
            });
            resolve();
        } else {
            ...
            setServerLanguage(message, client, flags).then(() => {
                main.sendMessage({
                    message, text: `The default laguage for ${message.guild.name} has been sets as ${language}`
                });
            });
        }
    }
