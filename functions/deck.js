...
const deck = async ({ message, params, flags }) => {
    ...
    await buildDeck(message, deck.shift(), flags);
};

const buildDeck = async (message, deck, flags) => {
    const language = getFlagLang(flags);
    const embed = new Discord.MessageEmbed();
    const dokStats = await fetchDoK(deck.id);
    const attachment = await buildDeckList({ ...deck, language });
    ...
    description += Object.keys(cardStats.card_type)
        .map(type => cardStats.card_type[type] ? `${cardStats.card_type[type]}${emoji(type.toLowerCase())}` : false)
        .filter(Boolean)
        .join(' • ') + '\n';
